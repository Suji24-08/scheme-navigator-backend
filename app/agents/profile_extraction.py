...
def profile_extraction_node(state: GraphState) -> dict:
    ...
    if result.get("error"):
        ...
        try:
            ...
            # Sanitizer in schemas.py should catch most messy output, but this is a last-resort
            # safety net so a single bad field never crashes the whole graph.
            profile = UserProfile(raw_text=state.raw_input, ambiguous_fields=["ALL"],
                                   extraction_notes=f"Profile validation failed unexpectedly: {e}")
